...
public class Level {
	...
	private void water(int col, int row, Map map, int fullness) {
		...
		if(row + 1 < map.getHeight() && !map.getTiles()[col][row + 1].isSolid()) {
	...
	private void addGas(int col, int row, Map map, int numSquaresToFill, ArrayList<Gas> placedThisRound) {
		...
		int x = 0;
		int y = 0;
		Gas g = new Gas (col, row, tileSize, tileset.getImage(name), this, 0);
		...
		while (numSquaresToFill > 0 && placedThisRound.size() > 0) {
			...
			y = placedThisRound.get(index).getRow();
			if (y > 0 && !map.getTiles()[x][y - 1].isSolid() && !(map.getTiles()[x][y - 1] instanceof Gas) && numSquaresToFill > 0) {
				g = new Gas (x, y - 1, tileSize, tileset.getImage(name), this, 0);
				map.addTile(x, y - 1, g);
				placedThisRound.add(g);
				numSquaresToFill--;
				//breaky = false;
			}
			if (x > 0 && y > 0 && !map.getTiles()[x + 1][y - 1].isSolid() && !(map.getTiles()[x + 1][y - 1] instanceof Gas) && numSquaresToFill > 0) {
				g = new Gas (x + 1, y - 1, tileSize, tileset.getImage(name), this, 0);
				map.addTile(x + 1, y - 1, g);
				placedThisRound.add(g);
				numSquaresToFill--;
				//breaky = false;
			}
			if (x < map.getWidth() && y > 0 && !map.getTiles()[x - 1][y - 1].isSolid() && !(map.getTiles()[x - 1][y - 1] instanceof Gas) && numSquaresToFill > 0) {
				g = new Gas (x - 1, y - 1, tileSize, tileset.getImage(name), this, 0);
				map.addTile(x - 1, y - 1, g);
				placedThisRound.add(g);
				numSquaresToFill--;
			...
			if (x < map.getWidth() && !map.getTiles()[x + 1][y].isSolid() && !(map.getTiles()[x + 1][y] instanceof Gas) && numSquaresToFill > 0) {
				g = new Gas (x + 1, y, tileSize, tileset.getImage(name), this, 0);
				map.addTile(x + 1, y, g);
				placedThisRound.add(g);
				numSquaresToFill--;
